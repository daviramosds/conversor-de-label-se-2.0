...
function Clean() {
  ...
  if (document.getElementById("inputOutput1") != null) {
    document
      .getElementById("inputOutput1")
      .parentNode.removeChild(document.getElementById("inputOutput1"));
  }
}
...
function Verify() {
    ...
    if (label !== undefined && operadora !== undefined) {
      const operadoraFound = Ndata.operadoras.find(op => op.nome === operadora);
      
      if (operadoraFound) {
        const isSpecialOperator = [
          "VALECARD", "GOODCARD", "ECXCARD", "PLANVALE", 
          "LIBERCARD", "USECRED", "FAMILLYCARD", "PICPAY"
        ].includes(operadora);
  
        const operadoraId = operadoraFound.id;
        const operadoraLayouts = Ldata.operadoras.find(op => op.id === operadoraId);
        const { layout, layout2 } = operadoraLayouts || {};
  
        if (isSpecialOperator) {
          if (isSpecial) {
            isSpecial = false;
            createInputElement("inputOutput1");
          }
          updateSpecialDIV(layout, layout2);
        } else {
          isSpecial = true;
          removeSpecialInput();
          updateDIV(layout);
    ...
    function createInputElement(id) {
      ...
      newOutput.setAttribute("name", "output1");
      newOutput.setAttribute("size", "20");
      newOutput.setAttribute("readonly", "true");
  
      const referenceNode = document.getElementById("inputOutput");
      referenceNode.parentNode.insertBefore(newOutput, referenceNode.nextElementSibling);
    }
  
    function removeSpecialInput() {
      const specialInput = document.getElementById("inputOutput1");
      if (specialInput) {
        specialInput.parentNode.removeChild(specialInput);
      }
    }
  
    function updateDIV(layout) {
      document.convert.output.value = layout;
      const operadoraCode = Ndata.operadoras.find(op => op.nome === operadora).cod;
      document.getElementById("tempOutput").innerHTML = `${operadora} - ${operadoraCode}`;
      document.getElementById("inputOutput").select();
      document.execCommand("copy");
      document.getElementById(lastInputId).focus();
    }
  
    function updateSpecialDIV(layout, layout2) {
      document.convert.output.value = layout;
      document.convert.output1.value = layout2;
      const operadoraCode = Ndata.operadoras.find(op => op.nome === operadora).cod;
      document.getElementById("tempOutput").innerHTML = `${operadora} - ${operadoraCode}`;
    }
}
...
const handleGetAdquirentes = () => {
  ...
  adquirintes.forEach((adquirinte) => {
    const isFoundNdata = Ndata.operadoras.some(
      (op) => op.nome === adquirinte.toUpperCase()
    );
  ...
  document.getElementById(
    "notFoundAdquirintes"
  ).innerHTML = `Adquirintes não existentes: ${notFoundAdquirintesString}`;
};